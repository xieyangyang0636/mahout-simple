 ...
 */
public class UserCFLocal {

    private static Logger log = LoggerFactory.getLogger(UserCFLocal.class);
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            /**
             *  相似邻居计算
             *  1、固定数量的邻居         NearestNUserNeighborhood
             *      a、用“最近”的K个用户或物品最为邻居
             *      b、得到的邻居个数固定
             *      c、在数据稀疏的情况下，会出现较大的误差
             *  2、基于相似度门槛的邻居   ThresholdUserNeighborhood
             *      a、对邻居的远近进行最大值的限制，落在以当前点为中心，距离为 K 的区域中的所有点都作为当前点的邻居
             *      b、得到的邻居个数不确定
             *      c、不会出现较大的误差
             */
            NearestNUserNeighborhood neighbor = new NearestNUserNeighborhood(NEIGHBORHOOD_NUM, userSimilarity, dataModel);
            /**
             ...
             */
            Recommender recommender = new GenericUserBasedRecommender(dataModel, neighbor, userSimilarity);
            // 输出推荐结果
